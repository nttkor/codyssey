...
def tokenizer(expression):
    """수학 표현식 문자열을 토큰으로 분할합니다."""
    # 숫자 (음수 포함), 연산자, 괄호를 찾기 위해 정규식을 사용합니다.
...
def infix_to_postfix(tokens):
    """중위 표기법 토큰 목록을 후위 표기법 토큰 목록으로 변환합니다."""
    precedence = {'+': 1, '-': 1, '*': 2, '/': 2}
    output = []
    operator_stack = []
    ...
    for token in tokens:
        if re.match(r'-?\d+\.?\d*', token):
            ...
            if operator_stack and operator_stack[-1] == '(':
                ...
                raise ValueError("잘못된 표현식: 괄호가 일치하지 않습니다.")
