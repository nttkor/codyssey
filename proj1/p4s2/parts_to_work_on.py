...
def load_csv():
    try:
        ...
        parts_list = np.loadtxt(file_path_1, delimiter=',',skiprows=1,usecols=0, dtype=str)
        ...
        print(f"오류: 파일을 찾을 수 없습니다 - {e.filename}")
        return None, None
def get_parts_mean(parts):
    # 'parts' 배열이 성공적으로 생성되었을 때만 실행
    if parts is not None:
        # 항목별(열별) 평균값 계산
        column_means = np.mean(parts, axis=1)
        print("항목별 평균값:")
        print(column_means)
        print("---------------------------------")
        return column_means
...
def get_less_then(arr, value):
        # 조건에 맞는 열만 필터링하여 새로운 배열 생성
        mask = arr < value
        parts_to_work_on = arr[mask]
...
def main():
    ...
    # parts = load_csv()
    parts_list, parts = load_csv()
    parts_mean = get_parts_mean(parts)
    parts_filterd = get_less_then(parts_mean, 50)
