# -*- coding: utf-8 -*-
"""
engineering_calculator_lut.py
=============================

개요
----
이 모듈은 **PyQt6 기반 공학용 계산기**의 GUI/엔진을 포함한다. UI는 Qt Designer에서
작성한 `engineering.ui`를 **런타임에 loadUiType()**으로 로드하여 사용한다.

설계 원칙
- **UI 고정 전제**: 버튼의 objectName 규칙(`btnn_*`, `btno_*`, `btnf_*`)을 이용해
  클릭 시그널을 자동 라우팅한다. (오토커넥트 미사용)
- **엔진/표시 분리**: GUI는 사람이 읽기 쉬운 기호(×, ÷, π, ², ³ 등)를 사용하고,
  평가 직전 내부 기호(*, /)로 변환한다.
- **파서 내장**: 정규식을 쓰지 않고 토큰화 → 단항 마이너스 보정 → 셔닝야드로 RPN →
  스택 평가 순으로 수식을 처리한다.
- **공학 기능**: π, sin/cos/tan, sinh/cosh/tanh, 후위연산(²/³/%) 지원. 메모리(MC/MR/M+/M-),
  Ans, Deg/Rad 토글을 제공한다.
- **코드 간략화 포인트**: `loadUiType()`만 도입하여 `findChild`와 라벨 탐색 보일러플레이트를
  제거했다. 기존의 규칙 기반 자동 라우팅은 유지한다.

사용법
- `engineering.ui` 파일이 본 파이썬 파일과 같은 디렉터리에 있어야 한다.
- `python engineering_calculator_lut.py`로 실행한다.

주의
- `QLineEdit`는 부분 색상 하이라이팅을 지원하지 않는다. (연산자만 색 변경을 원한다면
  `QLabel(리치텍스트)` 또는 `QTextEdit`으로 교체 필요)
"""
...
FIXED_DECIMALS: Optional[int] = None     # 고정 소수 자리수 (None이면 유효숫자 모드)

def fmt_number(x: float) -> str:
    """
    계산 결과 숫자를 보기 좋게 문자열로 포매팅한다.

    동작:
    - 정수처럼 떨어지는 실수는 정수 문자열로 반환
...
class Calculator:
    """
    사칙/괄호/상수/함수/후위연산을 처리하는 **표준 계산 엔진**.

    파이프라인:
    - `_tokenize` : 입력 문자열을 토큰 시퀀스로 변환
    - `_fix_unary_minus` : 단항 마이너스(선행 '-')를 0 - x 형태로 보정
    - `_to_rpn` : 셔닝야드 알고리즘으로 중위식 → RPN(후위표기) 변환
    - `_eval_rpn` : 스택 기반으로 RPN을 평가

    하위 클래스 훅(Hook):
    - `apply_function` : sin/cos/tan/sinh/cosh/tanh 등 함수 처리
    - `apply_postfix`  : ²/³/% 등 후위 연산 처리
    - `const_value`    : π 등 상수 처리
    """

    def __init__(self):
        """메모리와 마지막 결과(Ans)를 초기화한다."""
        self.memory = 0.0
        self.last_result: Optional[float] = None  # Ans
    ...
    def mem_add(self, x: float) -> None:
        """
        현재 메모리에 x를 더한다. NaN/inf 또는 None은 무시한다.
        """
        if x is not None and math.isfinite(x):
            self.memory += x

    def mem_sub(self, x: float) -> None:
        """
        현재 메모리에서 x를 뺀다. NaN/inf 또는 None은 무시한다.
        """
        if x is not None and math.isfinite(x):
            self.memory -= x
    ...
    def apply_function(self, name: str, x: float, angle_mode_rad: bool) -> float:
        """
        수학 함수를 적용한다. 서브클래스가 구현해야 한다.
        """
        raise NotImplementedError

    def apply_postfix(self, op: str, a: float) -> float:
        """
        후위 연산자를 적용한다(예: 제곱, 세제곱, 퍼센트). 서브클래스가 구현.
        """
        raise NotImplementedError

    def const_value(self, name: str) -> float:
        """
        상수 값을 반환한다(예: 'pi'). 서브클래스가 구현.
        """
        raise NotImplementedError
    ...
    def evaluate_expr(self, s: str, angle_mode_rad: bool) -> float:
        ...
        ----------
        s : str
            내부 연산 기호(*, / 등)로 변환된 표현식 문자열
        angle_mode_rad : bool
            삼각함수 입력의 각도 모드. True면 라디안, False면 도(deg).
    ...
    # ----- Tokenization -----
    def _tokenize(self, s: str) -> List[Tuple[str, object]]:
        """
        입력 문자열을 토큰 리스트로 변환한다. 정규표현식을 사용하지 않는다.

        토큰 종류: ('NUM', float), ('OP', '+-*/'), ('LPAREN','('),
                   ('RPAREN',')'), ('FUNC', name), ('CONST', name), ('POST', op)
        """
    ...
    def _fix_unary_minus(self, tokens: Iterable[Tuple[str, object]]) -> List[Tuple[str, object]]:
        """
        단항 마이너스를 0 - x 형태로 보정한다.
        (식 시작/연산자/여는괄호/함수 뒤의 '-'를 단항으로 간주)
    ...
    def _to_rpn(self, tokens: Iterable[Tuple[str, object]]) -> List[Tuple[str, object]]:
        """
        셔닝야드 알고리즘으로 중위식 토큰을 RPN(후위표기)으로 변환한다.
        """
        out: List[Tuple[str, object]] = []
        st: List[Tuple[str, object]] = []
        prec = {'+':1, '-':1, '*':2, '/':2}
    ...
    def _eval_rpn(self, rpn: Iterable[Tuple[str, object]], angle_mode_rad: bool) -> float:
        """
        RPN(후위표기) 토큰 시퀀스를 스택으로 평가한다.
        """
        st: List[float] = []
...
class EngineeringCalculator(Calculator):
    """
    공학 기능을 구현한 Calculator 서브클래스.

    지원 기능
    - 상수: π
    - 삼각함수: sin, cos, tan (Deg/Rad 모드 지원)
    - 쌍곡선: sinh, cosh, tanh
    - 후위연산: ², ³, %
    """

    def __init__(self):
        """기본 각도 모드를 Deg(False)로 설정한다."""
        super().__init__()
        self.angle_mode_rad = False  # Deg 기본

    def const_value(self, name: str) -> float:
        """지정된 상수 값을 반환한다. 현재는 'pi'만 지원한다."""
        if name == 'pi':
            return math.pi
    ...
    def apply_function(self, name: str, x: float, angle_mode_rad: bool) -> float:
        ...
        ----------
        name : str
            함수 이름 (sin, cos, tan, sinh, cosh, tanh)
        x : float
            인자값
        angle_mode_rad : bool
            삼각함수의 각도 모드 (True=라디안 / False=도)
        """
        n = name.lower()
...
# loadUiType으로 .ui에서 FormClass/BaseClass 생성
FormClass, BaseClass = uic.loadUiType(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "engineering.ui")
)
...
class MainWindow(BaseClass, FormClass):
    ...
    역할
    - 디스플레이(표시/결과)와 버튼 입력을 관리
    - objectName 규칙 기반의 **자동 라우팅**으로 핸들러를 바인딩
    - UX 정책(= 이후 입력, 암시적 곱셈, 함수 머리 통삭제, 괄호 자동 닫기 등) 구현
    """

    # 함수 머리 토막(DEL 시 통삭제용), 값 판정 문자 집합
    ...
    def __init__(self):
        """UI 로드 및 시그널 라우팅 초기화."""
        super().__init__()
        self.setupUi(self)  # Designer 위젯이 self.<objectName>로 바로 바인딩

        # 디스플레이 길이에 따른 폰트 자동 축소
        self.le_expr: QLineEdit
        ...
        self._pending_clear = False  # '=' 이후 첫 입력 처리 플래그
    ...
    def _fit_font(self, led: QLineEdit, text: str) -> None:
        """
        QLineEdit 안의 텍스트 길이에 따라 폰트 크기를 동적으로 조절한다.
        """
    ...
    def _set_text(self, s: str) -> None:
        """
        표시식을 강제로 설정한다.
        (중앙집중식 출력 경로: 추후 하이라이트/필터를 추가하기 용이)
        """
    ...
    def _insert_func(self, name: str) -> None:
        """함수 입력: 암시적 곱셈 적용 후 'name('을 붙인다."""
    ...
    # 사칙 입력(= 이후 Ans 시드 정책 적용)
    def do_plus(self) -> None:  self._seed_ans_then("+")
    def do_minus(self) -> None: self._seed_ans_then("-")
    def do_mul(self) -> None:   self._seed_ans_then("x")   # 표시용
    def do_div(self) -> None:   self._seed_ans_then("÷")   # 표시용

    def do_open_paren(self) -> None:
        """여는 괄호 입력(숫자 뒤면 'x(' 자동 삽입)."""
        self._clear_if_pending()
        self._append("x(" if self._is_digit_tail() else "(")

    def do_close_paren(self) -> None:
        """닫는 괄호 입력."""
    ...
    def do_rad(self) -> None:
        """Deg/Rad 토글 후 버튼 텍스트를 동기화한다."""
        self.engine.angle_mode_rad = not self.engine.angle_mode_rad
        self._sync_rad_button_text()

    def _sync_rad_button_text(self) -> None:
        """
        각도 모드에 맞게 'btno_rad' 버튼 라벨을 'Rad'/'Deg'로 설정한다.
        loadUiType 사용으로 직접 접근 가능.
        """
        if hasattr(self, "btno_rad") and self.btno_rad:
            self.btno_rad.setText("Rad" if self.engine.angle_mode_rad else "Deg")

    # ----- 메모리 -----

    def _current_value_for_memory(self) -> Optional[float]:
        """
        메모리 연산용 숫자를 가져온다.
        - 우선 결과창 텍스트 사용
        - 없거나 파싱 불가면 표시식을 평가
